...
// 分析文本文件
function analyzeTextFile(filePath, outputToFile = false) {
    try {
        ...
        output += `\n=== 簡體字檢查結果 ===\n`;
        output += `文件: ${filePath}\n`;
        output += `總字符數: ${content.length}\n`;
        output += `發現簡體字: ${results.length} 個\n\n`;
        ...
        if (results.length === 0) {
            ...
            results.forEach((result, index) => {
                output += `${index + 1}. 位置 ${result.position}:\n`;
                output += `   簡體字: ${result.simplifiedChar}   繁體字: ${result.traditionalChar}\n`;
                output += `   上下文: ${result.context.full}\n\n`;
            });
        }
        ...
        // 如果需要，同時輸出到文件
        if (outputToFile) {
            const outputFileName = '簡繁體檢查結果.txt';
            fs.writeFileSync(outputFileName, output, 'utf8');
            console.log(`\n結果已保存到文件: ${outputFileName}`);
        }
...
function main() {
    ...
    if (args.length === 0) {
        console.log('使用方法:');
        console.log('node simplified_traditional_checker.cjs <文本文件路徑> [--save]');
        console.log('');
        console.log('例如:');
        console.log('node simplified_traditional_checker.cjs extracted_text.txt');
        console.log('node simplified_traditional_checker.cjs extracted_text.txt --save  # 同時保存到文件');
        return;
    }
    ...
    analyzeTextFile(filePath, saveToFile);
}
